...
class ReplDriver(settings: Array[String],
   ...
   */
  def runUntilQuit(initialState: State = initialState): State = {
  ...
  final def runQuietly(input: String)(implicit state: State): State = runBody {
    val parsed = ParseResult(input)(state)
  ...
  protected def interpret(res: ParseResult, quiet: Boolean = false)(implicit state: State): State = {
    res match {
